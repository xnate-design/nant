...
export default function Link({ href, className, children, ...props }: JSX.IntrinsicElements['a']) {
  const classes =
  ...
  return (
    <>
      {href.startsWith('https://') ? (
        <ExternalLink href={href} className={clsx(classes, className)} {...props}>
          {children}
        </ExternalLink>
      ) : href.startsWith('#') ? (
        ...
        <InnerLink to={href} className={clsx(classes, className)}>
          {children}
        </InnerLink>
      )}
    </>
