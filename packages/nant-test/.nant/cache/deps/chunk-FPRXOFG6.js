import { StyleModule, base, keyName, shift } from './chunk-MHTA6FMV.js';
...
function materialize(cursor) {
  ...
  for (; ; i++) {
    ...
    if (off <= node.from && off + base2.children[i].length >= node.to) break;
  }
  let buf = base2.children[i],
    b = buf.buffer;
  function split(startI, endI, type, innerOffset, length) {
    let i2 = startI;
    while (b[i2 + 2] + off <= node.from) i2 = b[i2 + 3];
    let children = [],
      positions = [];
    ...
    for (let i = 1; i < ranges.length; i++) {
      ...
      if (range.empty ? range.from <= prev.to : range.from < prev.to) {
        ...
        ranges.splice(
          --i,
          2,
          range.anchor > range.head ? _EditorSelection.range(to, from) : _EditorSelection.range(from, to),
        );
      }
    }
    return new _EditorSelection(ranges, mainIndex);
    ...
    while (cur.value || i < add.length) {
      if (i < add.length && (cur.from - add[i].from || cur.startSide - add[i].value.startSide) >= 0) {
        ...
        if (!filter || filterFrom > cur.to || filterTo < cur.from || filter(cur.from, cur.to, cur.value)) {
          if (!builder.addInner(cur.from, cur.to, cur.value)) spill.push(Range2.create(cur.from, cur.to, cur.value));
        }
        cur.next();
      }
    }
...
// ../../node_modules/.pnpm/@codemirror+view@6.16.0/node_modules/@codemirror/view/dist/index.js
...
var TextView = class _TextView extends ContentView {
  ...
  merge(from, to, source) {
    if (
      this.flags & 8 ||
      (source &&
        (!(source instanceof _TextView) || this.length - (to - from) + source.length > MaxJoinLen || source.flags & 8))
    )
      return false;
...
function dropText(view, event, text, direct) {
  ...
  let { mouseSelection } = view.inputState;
  let del =
    direct && mouseSelection && mouseSelection.dragging && dragMovesSelection(view, event)
...
var HeightMapBranch = class extends HeightMap {
  ...
  lineAt(value, type, oracle, top, offset) {
    ...
    else return this.left.lineAt(rightOffset, subQuery, oracle, top, offset).join(base2);
  }
  forEachLine(from, to, oracle, top, offset, f) {
    let rightTop = top + this.left.height,
      rightOffset = offset + this.left.length + this.break;
...
var handleKeyEvents = Prec.default(
  ...
  combine: (values) => {
    ...
    return {
      position: browser.ios
        ? 'absolute'
        : ((_a2 = values.find((conf) => conf.position)) === null || _a2 === void 0 ? void 0 : _a2.position) || 'fixed',
...
var tooltipPlugin = ViewPlugin.fromClass(
  class {
    constructor(view) {
      this.view = view;
      this.inView = true;
...
var panelPlugin = ViewPlugin.fromClass(
  class {
    constructor(view) {
...
var gutterView = ViewPlugin.fromClass(
  class {
    constructor(view) {
      this.view = view;
        ...
        domEventHandlers(a, b) {
          ...
          for (let event in b) {
            let exists = result[event],
              add = b[event];
            result[event] = exists
              ? (view, line, event2) => exists(view, line, event2) || add(view, line, event2)
              : add;
          }
          return result;
        },
...
var whitespaceHighlighter = matcher(
  new MatchDecorator({
    regexp: /\t| +/g,
...
var trailingHighlighter = matcher(
  new MatchDecorator({
    regexp: /\s+$/g,
...
var Modifier = class _Modifier {
  ...
  static get(base2, mods) {
    ...
    for (let parent of base2.set)
      if (!parent.modified.length) for (let config of configs) set.push(_Modifier.get(parent, config));
    return tag;
  }
};
...
// ../../node_modules/.pnpm/@codemirror+language@6.9.0/node_modules/@codemirror/language/dist/index.js
var _a;
    ...
    this.extension = [
      ...
      EditorState.languageData.of((state, pos, side) => {
        let top = topNodeAt(state, pos, side),
          data2 = top.type.prop(languageDataProp);
        if (!data2) return [];
        let base2 = state.facet(data2),
...
//# sourceMappingURL=chunk-FPRXOFG6.js.map
