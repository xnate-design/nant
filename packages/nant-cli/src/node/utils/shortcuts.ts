...
export function bindShortcuts(server: ViteDevServer, createDevServer: CreateDevServer): void {
  if (!server.httpServer || !process.stdin.isTTY || process.env.CI) {
    return;
  }

  server.config.logger.info(c.dim(c.green('  ➜')) + c.dim('  press ') + c.bold('h') + c.dim(' to show help'));
    ...
    if (input === 'h') {
      server.config.logger.info(
        [
          ...
          c.bold('  Shortcuts'),
          ...SHORTCUTS.map(
            (shortcut) => c.dim('  press ') + c.bold(shortcut.key) + c.dim(` to ${shortcut.description}`),
          ),
        ].join('\n'),
      );
    }
